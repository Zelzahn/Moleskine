...
export default class PlayerCommand extends Command {
    ...
    async run(message) {
        ...
        if (candidats.length < 3) {
            ...
            embed.setDescription(`Het seizoen is geëindigd, ${players.filter(player => player.lastWeek === week)[0].name} was de mol en ${players.filter(player => player.lastWeek === week - 1)[0].name} heeft gewonnen`);
        } else {
